import LocalizedString from 'react-localization';


let string = new LocalizedString({
    en:{
        navigation:{
            home:"Home",
            venue:"Venue",
            cityinformation:"Travel Information",
            registration:"Registration",
            agenda:"Schedule",
            staff:"Staff",
            sponsor:"Sponsor",
            language:"Language",
            speakers:"Speakers",
            cfp:"Call for Proposal"
        },
        home:{
            news:{
                title:"News",
                tuanrumah:"Gresik, Tuan Rumah GNOME.Asia Summit 2019",
                tuanrumahcontent:"GNOME.Asia Summit merupakan acara rutin komunitas GNOME di Asia yang digelar setiap tahunnya. Kegiatan ini biasanya digelar selama beberapa hari dan mempertemukan para pengembang, pengguna, dan orang-orang yang memiliki antusiasme terhadap GNOME atau hal-hal terkait dengan tema free & open source software. Tak heran, ajang ini selalu dinanti-nanti oleh para teman-teman komunitas di Indonesia maupun Asia.",
                tuanrumahlink:"https://gnome.id/Gresik-Tuan-Rumah-GNOME-Asia-Summit-2019.html",
                logo:"GNOME.Asia Summit 2019 Logo Competition",
                logocontent:"GNOME.Asia Summit 2019 will be held in Gresik, Indonesia in October, 2019. We are looking for creative people to design logo for this year’s Summit. The contest is open from now until June 21, 2019 and the winner will be announced on June 30, 2019",
                logolink:"https://gnome.id/Gnome-Asia-Logo-Competition/",
                pengumumanjudul:'GNOME.Asia Summit 2019 Logo Contest Announcement',
                pengumumancontent:'GNOME.Asia Summit 2019 logo competition is over, the vote was in. The resulting jury has been decided the winner of this logo competition. The winner is “Fendi Prad” from Indonesia.',
                pengumumanlink:'https://gnome.id/pengumuman-pemenang-kontes-logo-gnome-asia-summit-2019.html',
                cfpjudul:'Call for Papers for GNOME.Asia Summit 2019 Is Now Open! ',
                cfpcontent:'GNOME.Asia Summit 2019 invites you to participate as a speaker at the conference on the 12th and 13th of October 2019 in Gresik, Indonesia.GNOME.Asia Summit is the featured annual GNOME conference in Asia. It focuses primarily on the GNOME desktop, but also covers applications and platform development tools. The summit brings together the GNOME community in Asia to provide a forum for users, developers, foundation leaders, governments and businesses to discuss the present technology and future developments.',
                cfplink:'https://gnome.id/en/call-for-papers-for-gnome-asia-summit-2019-is-now-open/',
                regisjudul:'GNOME.Asia Summit 2019 Registration Now Open!',
                regiscontent:'GNOME.Asia Summit is the featured annual GNOME conference in Asia. It focuses primarily on the GNOME desktop, but also covers applications and platform development tools. The summit brings together the GNOME community in Asia to provide a forum for users, developers, foundation leaders, governments and businesses to discuss the present technology and future developments. The event, held at Universitas Muhammadiyah Gresik, will attract more than 200 participants and 34 speakers from local and international. There will be 4 amazing Keynote speakers in the event:',
                regislink:'https://gnome.id/en/gnome-asia-summit-2019-registration-now-open/',
                  
            }
        },
        etc:{
            more:"Read more..."
        },
        speakers:[
            {
                id: 1 ,
                name: "Aditya Manglik",
                topic: "Power management and measurement in GNOME",
                link: "#"
            },
            {
                id: 2 ,
                name: "Ahmad Haris",
                topic: "How To Contribute To FOSS Projects",
                link: "#"
            },
            {
                id: 3 ,
                name: "Ananda Dwi Rahmawati",
                topic: "Linkerd 2.0",
                link: "#"
            },
            {
                id: 4 ,
                name: "Andik Nur Achmad",
                topic: "Open Source in Homeschooling Family",
                link: "#"
            },
            {
                id: 5 ,
                name: "Andika Triwidada",
                topic: "Using Open Sorce to Manage Asian Games IT Security",
                link: "#"
            },
            {
                id: 6 ,
                name: "Bin Li",
                topic: "GNOME Shell introduction",
                link: "#"
            },
            {
                id: 7 ,
                name: "Budi Putra",
                topic: "Implementation and utilization file hosting Owncloud and virtualization Proxmox for Teaching Factory Model on SMK Muhammadiyah 1 Yogyakarta",
                link: "#"
            },
            {
                id: 8 ,
                name: "Darian Rizaludin",
                topic: "The experience of the KLAS roadshow to ignite the spirit of Open Source in East Java, Indonesia",
                link: "#"
            },
            {
                id: 9,
                name: "Ernawan Ernawan",
                topic: "Introducing and Educate Local Community to FOSS",
                link: "#"
            },
            {
                id: 10,
                name: "Faiq Aminuddin",
                topic: "Open Source and Education in Indonesia",
                link: "#"
            },
            {
                id: 11,
                name: "Fitra Aditya",
                topic: "Video Call Application using WebRTC and Gstreamer",
                link: "#"
            },
            {
                id: 12,
                name: "Florens Debora Patricia",
                topic: "Using Creative Commons License to Collaborate with Digital Artwork",
                link: "#"
            },
            {
                id: 13,
                name: "Franklin Weng",
                topic: "LibreOffice in Asia",
                link: "#"
            },
            {
                id: 14,
                name: "Gaurav Agrawal",
                topic: "Newcomers workshop (BoF)",
                link: "#"
            },
            {
                id: 15,
                name: "Hananto Herlambang",
                topic: "Guitar Shredding under GNOME Environment",
                link: "#"
            },
            {
                id: 16,
                name: "Herpiko Dwi Aguno",
                topic: "IRGSH",
                link: "#"
            },
            {
                id: 17,
                name: "Hui Wang",
                topic: "pulseaudio: improvement on audio streams switch",
                link: "#"
            },
            {
                id: 18,
                name: "Iwan Suryanto Tahari",
                topic: "Running Linux Desktop on Sony Playstation 4",
                link: "#"
            },
            {
                id: 19,
                name: "Kukuh Syafaat",
                topic: "GNOME l10n Indonesia Progress",
                link: "#"
            },
            {
                id: 20,
                name: "Radical Rakhman Wahid",
                topic: "Deep Learning in Linux Distros",
                link: "#"
            },
            {
                id: 21,
                name: "Rahman Yusri Aftian",
                topic: "Pegon at Gnome",
                link: "#"
            },
            {
                id: 22 ,
                name: "Rania Amina",
                topic: "Gimpscape: The Biggest Open Source Design Community in Indonesia",
                link: "#"
            },
            {
                id: 23,
                name: "Rizki Kelimutu",
                topic: "Managing social capital in open source",
                link: "#"
            },
            {
                id: 24,
                name: "Rosanna Yuen",
                topic: "GNOME Foundation — we're here to help",
                link: "#"
            },
            {
                id: 25,
                name: "Samsul Maarif",
                topic: "Current State of BlankOn Linux Development",
                link: "#"
            },
            {
                id: 26,
                name: "Sevagen Veerasamy",
                topic: "Human Interface Engineering (Icons and Graphic Design)",
                link: "#"
            },
            {
                id: 27,
                name: "Sofyan Sugianto",
                topic: "Collaboration in Game Development using Godot: an Open Source Software",
                link: "#"
            },
            {
                id: 28,
                name: "Sokibi Imgos",
                topic: "F/LOSS for Education",
                link: "#"
            },
            {
                id: 29,
                name: "Tobias Mueller",
                topic: "USB borne attacks and usable defense mechanisms",
                link: "#"
            },
            {
                id: 30,
                name: "Tonny Adhi Sabastian",
                topic: "Using Kafka and Elixir for Fault Tolerant Distributed Backend",
                link: "#"
            },
            {
                id: 31,
                name: "Uthia Estiane",
                topic: "The Role of Open Source Technology to Accelerate Sustainable Development Goals",
                link: "#"
            },
            {
                id: 32,
                name: "Yudhi Satrio",
                topic: "Understanding People with Visual Impairments Using Screen Reader (Gnome AT-API)",
                link: "#"
            },
            {
                id: 33,
                name: "Felipe Borges",
                topic: "Newcomers workshop (BoF)",
                link: "#"
            },
            {
                id: 34,
                name: "Sajeer Ahamad",
                topic: "Newcomers workshop (BoF)",
                link: "#"
            },
    ],
        notfound:"Page not Found"
    }
})

export default string;
